...
func newClientProgram(program string, settings ClientSettings) (*clientProgram, error) {
	...
	if err := resumeThread(&cp); err != nil {
		cmd.Process.Kill()
		cp.Close()
		cmd.Wait()
		return nil, fmt.Errorf("could not start new uci engine: %w", err)
...
func resumeThread(cp *clientProgram) error {
	snapshotHandle, err := windows.CreateToolhelp32Snapshot(windows.TH32CS_SNAPTHREAD, 0)
	if err != nil {
		return fmt.Errorf("could not resume thread: %w", err)
	}
	...
	if resumedCount == 0 {
		return fmt.Errorf("could not resume thread, could not resume any threads for process %d", cp.cmd.Process.Pid)
	}
